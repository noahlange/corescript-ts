import { PluginManager } from 'rpg_managers';
...
export default class ShaderTilemap extends Tilemap {
  ...
  public refreshTileset() {
    const bitmaps = this.bitmaps.map(
      (x: Bitmap) => (x.baseTexture ? new PIXI.Texture(x.baseTexture) : x)
    );
   ...
   */
  protected _hackRenderer(renderer: PIXI.WebGLRenderer): PIXI.WebGLRenderer {
    let af = this.animationFrame % 4;
    if (af === 3) {
      af = 1;
    }
    (renderer.plugins as any).tilemap.tileAnim[0] = af * this._tileWidth;
    (renderer.plugins as any).tilemap.tileAnim[1] =
    ...
    if (!this.lowerZLayer) {
      ...
      const useSquareShader = Number(
        parameters.hasOwnProperty('squareShader')
          ? (parameters as any).squareShader
          : 0
      );
